...
public class ArgumentFormatterUtils {
    ...
    public static Object[] getFormattedArgs(JoinPoint joinPoint, String[] argNames) {
        ...
        return Arrays.stream(argNames)
                .map(argName -> resolveArgument(argName, args, parameterNames)).toArray();
    }
    ...
    private static Optional<Object> findByParameterName(String argName, Object[] args,
    ...
    private static Object resolvePropertyPath(Object[] args, String path) {
        ...
        if (parts.length == 1) { // Try to match a full object name
            return Arrays.stream(args).filter(
                    arg -> arg != null && arg.getClass().getSimpleName().equalsIgnoreCase(parts[0]))
                    .findFirst().orElse("[unknown: " + path + "]");
        }

        // Find object matching the first part
        Object targetObj = Arrays.stream(args).filter(
                arg -> arg != null && arg.getClass().getSimpleName().equalsIgnoreCase(parts[0]))
        ...
        try {
            return PARSER
                    .parseExpression(String.join(".", Arrays.copyOfRange(parts, 1, parts.length)))
