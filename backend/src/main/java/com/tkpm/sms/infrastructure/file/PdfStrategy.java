...
public class PdfStrategy implements FileStrategy {
     ...
     * 
     * @param data the input data. It should be a List containing two elements: 1. The path to the
     * template file (String) 2. The data to be used in the template (Map<String, Object>)
     * 
        ...
        try {
            if (!(data instanceof List<?> dataList) || dataList.size() != 2) {
                throw new IllegalArgumentException(
                        "Data must be a List containing exactly 2 elements");
            }
            ...
            if (!(first instanceof String) || !(second instanceof Map)) {
                throw new IllegalArgumentException(
                        "First element must be String, second must be Map");
            }

            String templatePath = (String) first;
            @SuppressWarnings("unchecked") Map<String, Object> templateData = (Map<String, Object>) second;
            ...
            if (!templateData.containsKey("subjects")) {
                List<Map<String, Object>> subjects = List.of(
                        Map.of("courseId", "MATH101", "name", "Mathematics", "finalScore", 85.5),
                        Map.of("courseId", "LIT201", "name", "Literature", "finalScore", 92.0),
                        Map.of("courseId", "HIS301", "name", "History", "finalScore", 88.0),
                        Map.of("courseId", "PHY401", "name", "Physics", "finalScore", 90.0),
                        Map.of("courseId", "CHE501", "name", "Chemistry", "finalScore", 95.0),
                        Map.of("courseId", "BIO601", "name", "Biology", "finalScore", 89.0),
                        Map.of("courseId", "ENG701", "name", "English", "finalScore", 91.0));
            ...
            throw new FileProcessingException("Failed to export with PDF format",
