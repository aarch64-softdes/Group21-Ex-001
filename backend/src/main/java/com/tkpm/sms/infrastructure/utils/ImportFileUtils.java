...
public class ImportFileUtils {
    public static Address parseAddress(String address) {
        ...
        return Address.builder().street(parts[0]).ward(parts[1]).district(parts[2])
                .country(parts[3]).build();
    }

    public static Identity parseIdentity(String identityType, String identityNumber,
            String identityIssuedBy, String identityIssuedDate, String identityExpiryDate) {
        return Identity.builder().type(IdentityType.valueOf(identityType)).number(identityNumber)
                .issuedBy(identityIssuedBy).issuedDate(LocalDate.parse(identityIssuedDate))
                .expiryDate(LocalDate.parse(identityExpiryDate)).build();
    }
    ...
    public static AddressCreateRequestDto parseAddressCreateRequestDto(String address) {
        ...
        String[] parts = address.split(",");
        return AddressCreateRequestDto.builder().street(parts[0]).ward(parts[1]).district(parts[2])
                .province(parts[2] == null ? "" : parts[2])
                .country(parts[3] == null ? "" : parts[3]).build();
    }
    ...
    public static IdentityCreateRequestDto parseIdentityCreateRequestDto(
        ...
        return IdentityCreateRequestDto.builder().type(studentFileImportDto.getIdentityType())
