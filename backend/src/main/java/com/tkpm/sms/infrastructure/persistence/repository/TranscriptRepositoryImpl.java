...
public class TranscriptRepositoryImpl implements TranscriptRepository {
    ...
    @Override
    public AcademicTranscriptDto getAcademicTranscript(Student student, List<Enrollment> enrollments) {
                                    ...
                                    var subject = subjectRepository.findById(course.getSubject().getId())
                                            .orElseThrow(
                                                    () -> new ResourceNotFoundException(
                                                            String.format("Subject with id %s cannot be found", course.getSubject().getId())
                                                    )
                                            );
