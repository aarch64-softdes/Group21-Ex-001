...
public class IdentityValidator implements ConstraintValidator<IdentityConstraint, Object> {
    ...
    private final Map<IdentityType, ErrorCode> identityTypeErrorCodeMap = Map.of(
            IdentityType.CHIP_CARD, ErrorCode.INVALID_CHIP_BASE_NUMBER, IdentityType.IDENTITY_CARD,
            ErrorCode.INVALID_IDENTITY_CARD_NUMBER, IdentityType.PASSPORT,
    ...
    public boolean isValid(Object value, ConstraintValidatorContext context) {
        ...
        if (Strings.isEmpty(type) || Strings.isEmpty(number) || Objects.isNull(issueDate)
        ...
        if (issueDate.isAfter(expiryDate)) {
            context.buildConstraintViolationWithTemplate(
                    ErrorCode.INVALID_IDENTITY_ISSUED_DATE.name()).addPropertyNode("issuedDate")
    ...
    private void addValidationError(ErrorCode errorCode, String propertyType,
            ConstraintValidatorContext context) {
        context.buildConstraintViolationWithTemplate(errorCode.name()).addPropertyNode(propertyType)
                .addConstraintViolation();
    }
}